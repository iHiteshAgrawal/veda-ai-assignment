import { extractQuestionsFromLines } from "@/lib/ai";
import { handlePipelineRoute } from "@/lib/api";
import { extractQuestionsFromLinesBody } from "@/lib/schemas";
...
export async function POST(request: Request) {
  return handlePipelineRoute(
    request,
    extractQuestionsFromLinesBody,
    "Question extraction failed",
    async ({ lines }) => ({ questions: await extractQuestionsFromLines(lines) })
  );
}
